\chapter{Sets, Relations, and Axiomatic Foundations}
\renewcommand{\thesection}{\arabic{chapter}.\arabic{section}} % Reset section numbering for Chapter 1 onwards

\begin{learningobjectives}
By the end of this chapter, you should be able to:
\begin{itemize}
    \item work fluently with sets, Cartesian products, relations, functions, and orders;
    \item distinguish equivalence relations from partial and total orders;
    \item construct quotient sets and interpret fibers, images, and inverse images;
    \item explain why naive set formation leads to paradoxes and what role ZFC plays;
    \item decide when axiomatic set theory is relevant and when ordinary set notation is sufficient.
\end{itemize}
\end{learningobjectives}

\begin{mynote}
The ZFC material is a foundations module, not a prerequisite for ordinary calculus or linear algebra. Readers interested primarily in those subjects may read the sections on sets, functions, and relations, then continue to Chapter 2.
\end{mynote}

Set theory supplies a common language for modern mathematics, but it is not the only possible starting point and it should not be confused with the working content of every subject. In most undergraduate calculations, naive set notation is sufficient. Axiomatic set theory becomes important when we ask which collections are legitimate sets, how constructions are justified, or why unrestricted set formation leads to paradoxes. This chapter therefore separates practical set language from the optional study of its formal foundations.
...
\item Assume that the number of elements in the finite set $x$ is $\text{card}(x)$, then $\text{card}(\mathcal{P}(x)) = 2^{\text{card}(x)}$.

\item If the set $x$ is empty, then $\text{card}(\mathcal{P}(\emptyset)) = 1$. (This inference can be seen as an extension of the inference 1.1.2).
