...
\begin{itemize}
    \item model a discrete system as a graph and choose an appropriate graph representation;
    \item analyze degrees, connectivity, paths, cycles, trees, planarity, and coloring;
    \item execute basic traversal and shortest-path algorithms;
    \item distinguish deterministic graph theorems from probabilistic threshold statements;
    \item connect graph models with routing, networks, scheduling, and graph-based learning.
\end{itemize}
...
A connected graph $G$ has an Euler trail if and only if it has either zero or two vertices of odd degree. The trail is closed precisely when there are no odd-degree vertices; when there are exactly two, every Euler trail starts at one odd vertex and ends at the other.
\end{theorem}

\begin{proof}
In any trail, every internal visit to a vertex uses one entering and one leaving edge. Hence all vertices other than the two possible endpoints are incident with an even number of used edges. If the trail uses every edge, the graph can therefore have only zero or two odd-degree vertices.

Conversely, if every degree is even, the preceding Euler-circuit theorem applies. If exactly two vertices $u$ and $v$ have odd degree, add one temporary edge joining $u$ to $v$. Every degree in the enlarged connected graph is now even, so it has an Euler circuit. Removing the temporary edge breaks that circuit into an Euler trail in the original graph, beginning at one of $u,v$ and ending at the other.
\end{proof}
...
\begin{remark}[With high probability]
For a sequence of random graphs, an event occurs \textbf{with high probability} if its probability tends to $1$ as $n\to\infty$.
\end{remark}

\begin{theorem}[Giant-Component Phase Transition]
Fix $c>0$ and let $G\sim G(n,c/n)$.
\begin{itemize}
    \item If $0<c<1$, then with high probability every connected component has order $O(\log n)$.
    \item If $c=1$, the largest components have order $n^{2/3}$ in the critical scale.
    \item If $c>1$, then with high probability there is a unique giant component of order
    \[
        \rho n+o(n),
    \]
    where $\rho\in(0,1)$ is the nonzero solution of
    \[
        \rho=1-e^{-c\rho}.
    \]
    All remaining components have order $O(\log n)$.
\end{itemize}
\end{theorem}

\paragraph{Distances}
When the expected degree $np$ grows, a breadth-first search initially behaves like a branching process with approximately $np$ children per discovered vertex. This gives the useful heuristic
\[
    \text{typical distance}\approx\frac{\log n}{\log(np)}.
\]
Exact diameter statements depend on the regime of $p$ and require additional correction terms, so the heuristic should not be quoted as a universal theorem.

\paragraph{Clustering Coefficient}
\begin{definition}[Local Clustering Coefficient]
For a vertex $i$ of degree $k_i\ge2$, the local clustering coefficient is
\[
C_i = \frac{2e_i}{k_i(k_i-1)},
\]
where $e_i$ is the number of edges between neighbors of $i$.
\end{definition}

\begin{proposition}[Clustering in $G(n,p)$]
Conditional on the neighbor set of a vertex with degree at least two, every possible edge between two of its neighbors is present independently with probability $p$. Consequently,
\[
    \mathbb{E}[C_i\mid k_i\ge2]=p.
\]
This is one reason the basic Erdős--Rényi model often underestimates the high clustering observed in empirical networks.
\end{proposition}

\subsection{Important Theorems in Random Graph Theory}

\paragraph{Thresholds for Monotone Properties}
\begin{theorem}[Existence of a Threshold Function]
Let $\mathcal{P}$ be a nontrivial monotone increasing graph property. There is a function $p^*(n)$ such that
\[
\mathbb{P}(G(n,p)\in\mathcal{P})\longrightarrow
\begin{cases}
0, & p/p^*(n)\to0,\\
1, & p/p^*(n)\to\infty.
\end{cases}
\]
The theorem asserts the existence of a transition scale; determining its location and width is a separate problem.
\end{theorem}

\paragraph{Subgraph Appearance}
\begin{theorem}[Threshold for a Fixed Subgraph]
For a fixed graph $H$, define
\[
    m(H)=\max_{H'\subseteq H,\,v(H')\ge1}\frac{e(H')}{v(H')}.
\]
Then the threshold for $G(n,p)$ to contain a copy of $H$ is
\[
    p^*(n)=n^{-1/m(H)}.
\]
Thus the probability tends to $0$ when $p\ll p^*(n)$ and to $1$ when $p\gg p^*(n)$. If $H$ is strictly balanced, then $m(H)=e(H)/v(H)$.
\end{theorem}

\paragraph{Hamiltonicity}
\begin{theorem}[Hamiltonicity Threshold]
Let
\[
    p=\frac{\log n+\log\log n+c_n}{n}.
\]
Then
\[
\mathbb{P}(G(n,p)\text{ is Hamiltonian})\longrightarrow
\begin{cases}
0, & c_n\to-\infty,\\
1, & c_n\to+\infty.
\end{cases}
\]
The threshold coincides asymptotically with the disappearance of vertices of degree $0$ or $1$.
\end{theorem}
...
Connectivity thresholds in random graphs provide baseline models for robustness under independent vertex or edge failures. Real networks may behave differently because degree distributions, clustering, and dependencies are not captured by $G(n,p)$.
